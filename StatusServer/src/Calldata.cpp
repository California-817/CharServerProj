...
// 获取最小连接数的chatServer也有可能涉及到分布式锁的使用---使用分布式锁因为改连接数添加的是不是用户级别锁 而是系统级别锁 LOCK_COUNT
ChatServer_struct GetChatServerCalldata::getChatServer()
{
    ...
    auto minserver = _impl->_chat_servers.begin()->second;
    {
        ...
        if (!ret.has_value())
        {                                   // redis中没有这个chatServer的连接数缓存
            minserver._con_count = INT_MAX; // 设置成连接数量无穷大表示无效
        }
        else
        { // redis中获取最新的连接数(因为是由其他服务器进行修改,不能直接获取本地的conn数,不是最新的)
